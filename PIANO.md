...
### FASE 4ter — Copertura delle tre app — 10 sessioni + 6 di coda (`M4ter.11`–`M4ter.16`)
...
**M4ter.13 — Il picker d'inserimento voci di Studio (1 sessione)** — aggiunto il 2026-09-22, in coda a M4ter.12, con l'innesco già scattato. Ragione dichiarata: è **l'unica cosa che M4ter.12 ha lasciato scoperta** del Computo — quella sessione ha chiuso la griglia, non il modo in cui una voce ci entra dentro.

- **Obiettivo**: decidere se la schermata di scelta della lavorazione si **compone** con ciò che il registry ha già, o se è il secondo componente nostro. `SistemaPickerModal` di Studio: campo di ricerca, filtri di categoria a chip, risultati **raggruppati per famiglia** con conteggio dei prodotti e un collegamento «Vedi strati».
- **Prompt**: "**Non si parte dal presupposto che serva un blocco.** La scala di 4bis si esaurisce per intero e per iscritto, e il gradino 1 si chiede all'MCP: **`command`** di shadcn è una palette di comandi con ricerca e **gruppi**, cioè quasi la forma di questa schermata; `combobox` (M2.6) ha gruppi e ricerca ma è un **campo**, non una pagina di scelta; `tassullo-responsive-dialog` dà il contenitore, e sul telefono è già un cassetto; `toggle-group` è il chip che si clicca — e `CLAUDE.md` avverte proprio su questa coppia, che nel v1 confondere `badge` e `toggle-group` è costato riscritture. Se la composizione regge, **l'esito del task è una ricetta e una story**, non un item nuovo: è un risultato pieno e va scritto come tale. Si legge il `SistemaPickerModal` vero da un clone usa-e-getta via SSH, in sola lettura, cancellato a fine sessione. Quello che va misurato prima di decidere: **quante lavorazioni ha il catalogo** (se sono poche, la ricerca è un lusso; se sono molte, serve la virtualizzazione), quante **categorie**, se il raggruppamento è a uno o due livelli, e cosa fa «Vedi strati» — perché un collegamento dentro una voce d'elenco è un bersaglio dentro un bersaglio, ed è la stessa famiglia di `nested-interactive` che il gate ha preso in M4ter.12. **La tastiera si misura in Chromium vero**: una palette di comandi si usa **senza mouse** o non serve a niente — frecce fra i risultati che **saltano le intestazioni di gruppo**, `Invio` che sceglie, `Esc` che chiude riportando il fuoco al grilletto, e la ricerca che non perde il fuoco mentre si digita."
- **File**: *da decidere in sessione*. Se la composizione regge: una story e una riga nella guida. Se serve un blocco: `registry/tassullo/blocks/` più `registry.json`. In ogni caso `CHECKLIST.md`, `WORKLOG.md`, e `docs/ANALISI-COPERTURA-APP.md`.
- **Verifiche mie**: `npm run check` verde sui sette; `test:a11y` col conto nuovo **dichiarato** se si aggiungono scene (ogni scena vale 4 scansioni); `misura:bersagli` col controllo dello strumento; la tastiera provata in **Chromium vero**, non nel pannello (§22/§32). Se il picker ha un popup, va aggiunto a `CON_POPUP` di `scripts/gate-a11y.ts` **e** dichiarato con una `play`, o passerebbe per «senza popup».
- **Verifiche di Francesco**: si compone o serve un blocco? · Se si compone, la ricetta è abbastanza corta da essere seguita senza assistenza? · Il raggruppamento per famiglia con la domanda («Stai costruendo un edificio in muratura tradizionale?») è il modo giusto di scegliere, o è un residuo del v1?
- **Sub-agenti**: **no**, salvo la ricognizione in sola lettura sul clone di Studio.

**M4ter.14 — La testata di `Pagine/Lista` allineata a Prodotti (1 sessione)** — aggiunto **a cose fatte** il 2026-09-22, su rilievo di Francesco guardando la style guide pubblicata. Ragione dichiarata: le due pagine modello dello **stesso** elenco — `Pagine/Lista` (il blocco, M4.2) e `Pagine/Prodotti (Anagrafe)` (la pagina reale, M3bis.11b) — mostravano **tre forme diverse** della stessa cosa, e chi legge da fuori non ha modo di sapere quale sia quella buona. Sta in 4ter e non in FASE 5 per la ragione di M4ter.11–13: è **forma**, non documentazione, e M5.0 riscriverà il testo di quelle pagine dando per buona la forma che trova.

- **Obiettivo**: una forma sola per la testata di colonna e per i filtri di un elenco.
- **Esito**: `IntestazioneColonnaMenu` **esportata da `blocks/data-table.tsx`** — era copiata a mano in tre story — e resta **opt-in**, non il default di `IntestazioneColonna`. Su Lista: menu «⋮» al posto di tre controlli sempre accesi per colonna, `colonneRiordinabili` caduto come già su Prodotti, filtri sfaccettati su Categoria e Stato. Nessun item nuovo. Dettaglio e misure in `WORKLOG.md`, voce **M4ter.14**.

**M4ter.15 — La faccia stretta di `Pagine/Prodotti` (1 sessione)** — aggiunto il 2026-09-22 su mandato di Francesco: «per le pagine lista e prodotti va implementata anche la vista mobile, come per il computo o per la *lista a due facce*, con soglia della pagina. **Proviamo prima per prodotti e se validato estendiamo** anche alla pagina lista».

- **Obiettivo**: la seconda faccia della pagina reale, senza un blocco nuovo.
...
**M5.0 — La style guide per chi legge da fuori (5 sessioni: M5.0a–e)** — aggiunto in testa alla fase il 2026-09-22, su indicazione di Francesco, dopo aver guardato insieme la style guide pubblicata prima di aprire la FASE 5. Ragione dichiarata: **la style guide è il primo documento che un esterno apre**, e oggi non è leggibile da fuori — deve dire quale design si usa e come si applica, e invece contiene le note interne. Sta in FASE 5 e non in coda alla 4ter perché è documentazione, non forma: nessun componente cambia.

- **La ricognizione che l'ha aperta (2026-09-22, sola lettura)**: Storybook rende come prosa **ogni JSDoc attaccato a `const meta` o a `export const Story`**, più i `.mdx` e il JSX delle pagine `Tema/*`. Su 89 file di story, **87 hanno testo visibile con annotazioni interne**: **~750 rimandi** su **~7.000 righe** di prosa visibile (3.599 in descrizioni di componente, 3.359 in descrizioni di story). Non sono note sparse: le descrizioni **sono** il diario di sessione — sigle `M*`/`D*`/`§`, date, «misurato», «in carico a M2.9», nomi, rimandi a `WORKLOG.md`/`PIANO.md`/`DECISIONI.md`/`CLAUDE.md`/`INTERFACCE.md`, `check:registry`, «il gate». **La misura è del mattino e la giornata l'ha già superata**: M4ter.14/15/16 hanno riscritto quattro story e i file di story sono passati da 89 a **90**. M5.0a **rimisura**, non eredita il ≈740. I casi più esposti: `stories/Introduzione.mdx` (la pagina d'ingresso parla di M2.9 e della CI), `Tema/Carattere` e `Tema/Cifre` (prosa JSX con date, Roberto, §14/§48), `Pagine/Prodotti (Anagrafe)`. Puliti: `label`, `radio-group`, `Tema/Densità`, le etichette della barra in `.storybook/preview.tsx`. I commenti `//` e `/* */` **non attaccati** al meta non sono visibili (`.storybook/main.ts`, il blocco in testa a `Tipografia.stories.tsx`, i sorgenti dei componenti) e non si toccano. Dettaglio in `WORKLOG.md`, voce del 2026-09-22.
- **Le quattro scelte di Francesco (2026-09-22)**: (1) **riscrittura** per chi legge da fuori, non sola rimozione dei marcatori; (2) le note interne **si cancellano** — sono già in `WORKLOG.md` e `docs/DECISIONI.md`; se una misura sta *solo* nella story, si sposta in `DECISIONI.md` prima di toglierla; (3) nei dati d'esempio **le app restano** (Anagrafe, Officina, Studio sono le app Tassullo e il design system è loro) e **le persone reali diventano fittizie** — `Francesco Sartori` e `Roberto Zanetti` → nomi inventati; (4) **ottavo gate** `check:storybook`, perché una regola senza gate si perde in poche settimane.
- **Il canone di ogni pagina** — si scrive una volta in M5.0a (in testa a `scripts/check-storybook.ts` e qui) e vale per tutte. Descrizione di **componente** (il JSDoc su `const meta`), nell'ordine, solo le sezioni che servono: **(1) cos'è e a cosa serve**, una frase, per funzione e non per aspetto («un'etichetta che si legge», «un filtro che si clicca»); **(2) quando si usa, quando no**, il confine col componente vicino (`badge`/`toggle-group`, `dialog`/`sheet`/`drawer`, `select`/`combobox`); **(3) come si installa**, `npx shadcn@latest add tassullo/tassullo-design-system-v2/<item>` (con `#v2.0.0` da M5.6); **(4) varianti e taglie**, cosa esiste, col nome della prop; **(5) regole d'uso**, quelle ancora valide, scritte come regole e non come storia — «il testo arancione usa `text-accent-ink`, mai `text-primary`», «i numeri in colonna vogliono `tabular-nums`», «i codici si scrivono nel carattere del testo, `text-sm text-muted-foreground`»; **(6) tastiera e accessibilità**, cosa il componente garantisce (frecce, `Esc`, dove va il fuoco), come comportamento e non come misura. Descrizione di **story**: una o due frasi su cosa mostra la scena e a cosa guardare. Niente «perché», niente «prima era».
- **Cosa non ci va, e il gate lo verifica**: sigle `M<n>.<n>` (anche `bis`/`ter`), `FASE <n>`, `D<n>`, `§<n>`; i nomi dei documenti interni (`WORKLOG`, `PIANO`, `CHECKLIST`, `DECISIONI`, `CLAUDE.md`, `INTERFACCE.md`, `ROADMAP`, `ANALISI-COPERTURA`, `componenti-propri`, `registry/.upstream`); date ISO e «n settembre 2026» in prosa; `Francesco`, `Roberto`; `npm run`, `check:`, «il gate», «CI»; «v1» e `styleguide.html`. Restano legittimi: shadcn, Base UI, Tailwind, reui, Recharts, WCAG, «axe» come nome dello strumento nella pagina Accessibilità. **Cosa non ci va ma lo decide chi scrive**, non il gate (darebbe falsi positivi, e un gate che dà falsi positivi si smette di leggere): «misurato», «sospeso», «per ora», «reversibile», «in carico a», «fino a … c'era scritto», «il gate ha fatto il suo mestiere». Sparisce anche ogni frase su cosa un'app fa *oggi* («Anagrafe lo ha già fatto a mano in `.prd-cerca`»); il `title: 'Pagine/Prodotti (Anagrafe)'` resta.
- **Il gate `scripts/check-storybook.ts`** (modello: `scripts/check-riferimenti.ts` — uscita 1, conteggi, autotest). **Legge solo il testo visibile**: (a) il JSDoc immediatamente prima di `const meta` o di `export const <Nome>` in ogni `*.stories.tsx` di `registry/tassullo/{ui,blocks,pages}` e `stories/`; (b) i `parameters.docs.description` scritti a mano; (c) il corpo dei `.mdx`; (d) i nodi di testo JSX nei soli `stories/*.stories.tsx`, che sono pagine di prosa. **Non** legge `//`, `/* */` staccati, le `play`, i sorgenti dei componenti. Segnala file, riga, espressione; non suggerisce niente, perché non c'è un «nome giusto». **Nessuna esenzione per token**: se un caso legittimo emerge, si stringe l'espressione. `-- --self-test`: un file finto con i sette tipi di violazione → 7; un file pulito → 0; un file con la sigla dentro un commento `//` → 0 (prova che non guarda l'invisibile). Costo atteso sotto il secondo, niente browser. Nasce in M5.0a **in modalità avviso** (`--avvisa`: stampa il conto e non fallisce), e passa a errore ed entra in `npm run check` e in `.github/workflows/gate.yml` in M5.0e.
- **Ogni sessione chiude con**: `build-storybook`; le pagine riscritte aperte nel browser, in chiaro e in scuro; `test:a11y` con conto **invariato a 1540/0 su 385 story** (le descrizioni non entrano nel canvas: se il conto si muove, si è toccato altro). *Aggiornato il 2026-09-22 a fine giornata: era 1524/0 su 381 quando M5.0 è stata scritta, e M4ter.15/16 hanno aggiunto quattro scene — le due facce strette e le due di soglia. Se altre scene arrivano prima della FASE 5, il conto **si rilegge**, non si eredita.*; la riga in `CHECKLIST.md` e la voce in `WORKLOG.md`.
- **Sub-agenti**: **sì, per la ricognizione** (è già stata fatta così); **no per la scrittura** — il canone si tiene in una mano sola, o le 89 pagine escono in cinque toni.

**M5.0a — Il canone, l'ingresso e il tema, lo scheletro del gate (1 sessione)**
- Prompt: "Scrivi il canone di pagina in testa a `scripts/check-storybook.ts` e costruisci il gate in modalità avviso, con `--self-test`. Poi riscrivi secondo il canone `stories/Introduzione.mdx` — cos'è la style guide, le tre sezioni, i tre interruttori, come si installa un componente, l'accessibilità come garanzia e non come cronaca — e le pagine `Tema/*` (`Carattere`, `Cifre`, `Palette`, `Tavolozza`, `Tipografia`, `Densita`), che sono prosa JSX: lì si spiega il carattere, la scala, la palette e le due densità come si spiegherebbero a chi deve usarli, senza date, nomi, paragrafi e `§`. Le misure che stanno solo lì (i 2,69px/0,04px dei decimali, i pesi resi, la ΔE della tavolozza) si cercano in `docs/DECISIONI.md` e, se mancano, ci si portano prima di toglierle."
- File: *nuovo* `scripts/check-storybook.ts`; *modificati* `package.json` (script `check:storybook`, ancora fuori da `check`), `stories/Introduzione.mdx`, `stories/{Carattere,Cifre,Palette,Tavolozza,Tipografia,Densita}.stories.tsx`, `docs/DECISIONI.md` se serve.
- Accettazione: `check:storybook -- --self-test` 7/0/0; le 7 pagine a **0 segnalazioni**; il gate stampa il conto sul resto del repo (atteso ≈740, **da rimisurare**: v. sopra) senza fallire; `test:a11y` 1540/0.

**M5.0b — Primitive, prima metà (1 sessione)**
- *Rettifica di M5.0a (2026-09-22): in ordine alfabetico `accordion`…`item` sono **28** file e `kbd`…`tooltip` **24**, non 26 + 26 — il taglio resta su `item`/`kbd`, cambiano i conti. E un rilievo da decidere aprendo la sessione: `tags: ['autodocs']` è acceso su **3** file soli (`calendario`, `data-grid`, `data-table`), quindi il JSDoc di `meta` — dove il canone mette «cos'è, quando, `add`» — oggi un visitatore **non lo vede** sulle altre 49 primitive: sta solo nel bundle. **Deciso e fatto in coda a M5.0a** (Francesco): `tags: ['autodocs']` globale in `.storybook/preview.tsx`, 90 pagine Docs, `test:a11y` invariato a 1540/0. Da qui il JSDoc di `meta` è la prima cosa che un visitatore legge su ogni componente.*
- Prompt: "Le 26 story di `registry/tassullo/ui/` da `accordion` a `item` in ordine alfabetico, riscritte secondo il canone. Per ognuna: cos'è, quando sì e quando no, il comando `add`, varianti e taglie, le regole d'uso ancora valide, tastiera. Prima di cancellare un paragrafo con un numero, si cerca il numero in `WORKLOG.md` e `docs/DECISIONI.md`."
- File: `registry/tassullo/ui/{accordion … item}.stories.tsx`.
- Accettazione: `check:storybook` a **0 segnalazioni** sui 26; `test:a11y` 1540/0.

**M5.0c — Primitive, seconda metà (1 sessione)**
- Prompt: "Come M5.0b, le 26 story da `kbd` a `tooltip`. Qui stanno le più pesanti — `select`, `sidebar`, `stepper`, `toggle-group`, `table` — e la trappola `badge`/`toggle-group` va scritta una volta, come regola, nelle due pagine."
- File: `registry/tassullo/ui/{kbd … tooltip}.stories.tsx`.
- Accettazione: `check:storybook` a **0 segnalazioni** sui 52 file di `ui/`; `test:a11y` 1540/0.

**M5.0d — Blocchi (1 sessione)**
- Prompt: "Le 21 story di `registry/tassullo/blocks/`. `data-table` (291 righe di JSDoc), `foglio-gruppi` (288) e `calendario` (203) sono le tre più lunghe e vanno riscritte per chi deve *comporre* la pagina: quali prop, quali slot, quale blocco vicino scegliere. Le note per un backend (inizio/fine esclusiva del calendario, `id` stabile) restano, perché sono istruzioni d'uso, ma senza il nome dell'app da cui vengono."
- **Da scrivere in evidenza, richiesta di Francesco il 2026-09-22** (nasce da M4ter.14): la story di `data-table` deve spiegare **`altezza="ferma"`** — cioè il riquadro che si ferma su un confine di riga e apre lo scorrimento interno — a chi compone la **prossima** pagina lista, quale che sia la sua paginazione: `perPagina="infinito"` come Prodotti, una `perPagina` numerica come Norme, o `"virtuale"`. Non è una nota di manutenzione: è la prop che decide se il piè di paginazione resta in vista, e il difetto che ne nasce quando la misura è sbagliata — **il riquadro che si rimpicciolisce filtrando e non ricresce più** — è un *cricchetto*, quindi si vede solo componendo una sequenza di gesti e non guardando una schermata. Lo stesso difetto è stato **segnalato su Prodotti nel 2026-09-18 e archiviato come "non riprodotto"**, perché la prova di allora filtrava fino a *una* riga: con una riga il contenitore non può scorrere, e senza scorrimento il difetto non esiste. Va scritto quindi: (a) che cosa fa `altezza="ferma"`, (b) che la misura del tetto è **relativa alle righe fra loro** e mai al contenitore, perché `getBoundingClientRect` porta dentro lo scorrimento, (c) la sequenza che lo riproduce — filtrare a **una manciata** di righe, non a una, poi togliere il filtro — così chi la incontra la riconosce invece di ripartire da zero. Le tre forme di paginazione vanno nominate tutte e tre: il difetto è dello stesso effetto, ma si manifesta solo dove il numero di righe può scendere sotto l'altezza della pagina.
- **E `IntestazioneColonnaMenu`, esportata in M4ter.14**: la seconda forma di testata del blocco — un solo grilletto «⋮» con Ordina + Blocca — va scritta come **alternativa opt-in** e non come il default, insieme a `meta.azioniProprie` senza cui i grilletti di pin tornano due. Oggi il suo JSDoc sta nel blocco, non nella story.
- File: `registry/tassullo/blocks/*.stories.tsx`.
- Accettazione: `check:storybook` a **0 segnalazioni** sui 21; `test:a11y` 1540/0; **la story di `data-table` nomina `altezza="ferma"`, la regola della misura relativa e la sequenza che riproduce il cricchetto**.

**M5.0e — Pagine, il gate armato, il documento pubblicato (1 sessione)**
- Prompt: "Le 6 pagine modello di `registry/tassullo/pages/` e le 4 pagine composte in `stories/` (`PaginaProdotti`, `ListaDueFacce`, `SceltaDaCatalogo`, `SchermateAccesso`): ogni pagina dice quali blocchi la compongono e con quale comando si installa, non da quale pagina di quale app è stata ricavata. Sostituzione globale dei nomi di persona nei dati d'esempio. Poi il gate passa a errore, entra in `npm run check` come ottavo e in `.github/workflows/gate.yml`; `README.md` §I gate e `CLAUDE.md` §Comandi lo descrivono. Infine `build-storybook` e un `grep` dei token vietati su `storybook-static/`, che è l'artefatto che il visitatore legge davvero."
- **E le due facce, aggiunte in M4ter.15/16**: `PaginaLista` ha ora `facciaStretta`, `soglia` e `faccia`, ed è **API che le app installano** — va scritta come si scrive una prop, non come il racconto di come ci si è arrivati. La regola da far passare è la divisione: il blocco sceglie *quando*, la pagina scrive *cosa*. Le scene nuove sono quattro (`Faccia stretta` e `Soglia della pagina` su `Pagine/Lista` e su `Pagine/Prodotti`), e `Pagine/Lista a due facce` resta la ricetta a cui rimandano.
- File: `registry/tassullo/pages/*.stories.tsx`, `stories/{PaginaProdotti,ListaDueFacce,SceltaDaCatalogo,SchermateAccesso}.stories.tsx`, `package.json`, `.github/workflows/gate.yml`, `README.md`, `CLAUDE.md`, `scripts/check-storybook.ts`.
- Accettazione: `npm run check` verde sugli **otto**; `grep -rlE "M[0-9](bis|ter)?\.[0-9]+|WORKLOG|DECISIONI|PIANO\.md|CLAUDE\.md|Francesco|Roberto" storybook-static/` → **nessun file**; Pages aggiornato e riletto dall'URL pubblico come se non si conoscesse il repo: `Introduzione`, tre pagine Tema, tre primitive, due blocchi, una pagina modello rispondono a «cos'è, quando, come si installa».

**M5.1 — `registry.json` completo** — *diviso in tre il 2026-09-23 con Francesco, dopo la misura: 90 note interne in `description`/`docs` di 38 item e 596 nei commenti di 53 file spediti non stavano in una sessione insieme ai due gate. Poi la misura con la CLI vera ha mostrato che `shadcn add` toglie i commenti di testa (`docs/DECISIONI.md` §58): di quelle 596, 251 all'app non arrivano, e restano note di repo. Il prompt originale — `validate` verde, `list` completo, `target` espliciti, `registryDependencies` coerenti — era già soddisfatto da M4.6.*

**M5.1a — Gate delle dipendenze e del testo spedito (1 sessione)** — *chiusa il 2026-09-23.*
...
**M5.2 — Prova d'installazione end-to-end (1 sessione)** — *chiusa il 2026-09-24, insieme a M5.3. Rivista il 2026-09-22: l'installazione in un'app Vite vuota è già stata fatta e cronometrata due volte (M4.6, M4ter.10: 36s di comandi, quattro prerequisiti nel `README`), dal raw GitHub e non dal locale. Si fonde con l'accettazione di M5.3: la prova che conta è quella fatta seguendo **solo** `INTEGRAZIONE.md`.*
- Prompt: "App Vite usa-e-getta in una cartella temporanea: `npx shadcn@latest init`, poi `add` del tema e di un blocco dal registry locale, prima con `--dry-run` poi reale. Verifica che i file atterrino dove devono, che il CSS del tema sia importato nel CSS globale dell'app, che l'app compili e si veda in stile Tassullo. Ogni attrito d'uso va in WORKLOG: è il primo collaudo dell'esperienza di adozione."
- Accettazione: app di prova compilata e in stile; lista attriti nel worklog.

**M5.3 — `INTEGRAZIONE.md` v2: agganciare una app nuova (1 sessione)** — *chiusa il 2026-09-24, nella stessa sessione di M5.2.*
...
**M5.4 — Le regole da mettere nel `CLAUDE.md` e nei piani delle app (1 sessione)** — *chiusa il 2026-09-24. Scostamento: le regole dell'app installano con `@tassullo/<item>`, che prende la versione dal solo `components.json`; la forma lunga con `#` resta nel documento per i passi prima del registry (`WORKLOG.md`).*
...
- Accettazione: il blocco è copiabile senza adattamenti in un `CLAUDE.md`; `DECISIONI.md` spiega perché non npm, perché Base UI, perché `primary` ≠ `accent`, perché oklch, perché Inter si distribuisce dentro il tema in data URI.

**M5.5 — `GUIDA-MIGRAZIONE.md`: portare una app esistente sul v2 (1 sessione)** — *divisa in **M5.5a** e **M5.5b** il 2026-09-24, con Francesco. **M5.5a**: in un clone temporaneo di Anagrafe, scollegato da GitHub, si legge il codice (le pagine sono 19, non 10) e si misura la convivenza. Aggiungere Tailwind e il tema v2 cambia gli stili di base di tutte le pagine — l'azzeramento di Tailwind, sfondo, testo e carattere sul `body` —, quindi «i due temi convivono» vale per i nomi dei token e va verificato per la resa. **M5.5b**: la guida e il passo 0, **provato nel clone** e non nel repository vero. Il repository di Anagrafe non si tocca in FASE 5 (decisione di Francesco, stesso giorno): la transizione delle app esistenti, compresa la PR del passo 0, si pianifica a FASE 5 finita. Quella PR seguirà le regole di Anagrafe (ramo, PR, squash; ogni push su `main` va in produzione) e la approva Francesco. La guida dice anche come i documenti di Anagrafe (`CLAUDE.md`, `docs/INTERFACCE.md`, roadmap, checklist) passano dalle regole v1 al blocco v2: per un periodo convivono, pagina migrata per pagina migrata. **Quelle modifiche si preparano in M5.5b e si caricano all'avvio della transizione.** Si tengono in una cartella locale fuori da ogni repository, `~/Documents/Claude Code/transizione-anagrafe/` (Francesco), perché questo repository è pubblico e i documenti di Anagrafe parlano di cose interne (Business Central, Azure, ruoli). Qui restano solo i blocchi generici della guida. Il testo qui sotto resta il riferimento per il contenuto della guida. **Rettifica di M5.5a** (`docs/DECISIONI.md` §64): «i due temi convivono senza collisioni di nomi» non regge — venti nomi del tema di Tailwind coincidono con quelli del v1, e l'azzeramento cambia tutte le pagine v1. La convivenza si otterrebbe con la ricetta D di §64. **Ma su Anagrafe non si fa convivere** (Francesco, 2026-09-24, §64 «Seguito»): il passaggio al v2 si fa tutto su un ramo, il v1 si toglie nello stesso lavoro e in produzione arriva l'app finita. La migrazione pagina per pagina, l'ordine «tema affiancato → una pagina alla volta» e il «commit isolato per pagina» del prompt qui sotto non valgono per Anagrafe: M5.5b scrive la guida per il passaggio in un colpo solo.* *Chiusa il 2026-09-24: `docs/GUIDA-MIGRAZIONE.md`, passo 0 provato nel clone (`docs/DECISIONI.md` §65).*
- Prompt: "Le app dello studio **non si migrano ora** (vedi §3): questa è la guida che permetterà di farlo quando ognuna deciderà, senza riaprire l'analisi da capo. Contiene, come **passo 0 separato dalla migrazione e da fare subito**, il collegamento dell'MCP: in ogni app esistente si aggiunge un `components.json` minimo che dichiara il registry Tassullo v2 e si esegue `npx shadcn@latest mcp init --client claude`. È additivo e reversibile — non tocca il codice, non tocca gli stili, non richiede Tailwind — e da quel momento chi lavora su Anagrafe può **cercare e leggere** i componenti del v2 mentre sviluppa, invece di riscriverli. Installarli richiederà Tailwind e quindi la migrazione vera; consultarli no. L'indirizzo del registry è `tassullo/tassullo-design-system-v2`, con il tag (`…/<item>#v2.0.0`): il percorso di file locale non serve, perché l'MCP non sa sfogliare un registry da `file://` (`docs/DECISIONI.md` §9). Poi la guida vera e propria: la tabella token v1→v2 e la tabella classe v1→componente v2 (`.btn`→`button`, `.chip`→`toggle-group`, `.badge`→`badge`, `.card`→`card`, `.input`→`field`+`input`, `.alert`→`alert`, `.table`→`data-table`, `.sidebar`→`app-shell`, `.skel`→`skeleton`); la dimostrazione che **i due temi convivono** senza collisioni di nomi (il v1 usa `--color-*`, il v2 `--primary`/`--background`), che è ciò che rende la migrazione incrementale e interrompibile; l'ordine consigliato (Tailwind → tema affiancato → app-shell → una pagina lista e una di dettaglio come piloti → il resto → rimozione del v1, di `.stylelintrc.cjs` e dello step `lint:css`); la regola che ogni pagina migrata è **un commit isolato**, così ci si può fermare a metà con l'app funzionante; e l'avvertenza che se durante una migrazione emerge una lacuna del design system, si corregge **nel registry**, mai nell'app. **E tre voci che M4ter.11 ha trovato leggendo le app vere** (`docs/ANALISI-COPERTURA-APP.md` §8): **(a)** `window.confirm` e `window.prompt` sono ancora vivi in Anagrafe — `ChangeSets.tsx`, «Scrivere queste modifiche su Business Central?» e «Motivo della respinta:» — e `tassullo-confirm-dialog` li copre tutti e due, col campo obbligatorio e con la parola da ricopiare: è **l'unico pattern che si trova a `grep` e si chiude senza ragionare**, quindi va scritto come voce di ricerca-e-sostituzione; **(b)** il **Computo di Studio non si migra al primo giro** e va nominato fra le pagine che restano sul v1 — è in carico a `M4ter.12`, e la migrazione incrementale esiste apposta per poterlo lasciare indietro; **(c)** la soglia del bivio tabella/schede la decide l'app: `useSoglia(query)` non porta un numero, e la guida deve dire **come si sceglie**, non quale usare."
...
**M5.6 — Pubblicazione su GitHub (1 sessione)** — *chiusa il 2026-09-24: tag `v2.0.0`, prova da fuori riuscita (`WORKLOG.md`). Prima: non più bloccata: D4 è chiusa e il repo è online dal 2026-09-10, pubblico, con Pages attivo. Quello che resta al task è il **tag `v2.0.0`** e la **prova d'installazione dall'esterno**.*
