...
**M5.5 — `GUIDA-MIGRAZIONE.md`: portare una app esistente sul v2 (1 sessione)** — *divisa in **M5.5a** e **M5.5b** il 2026-09-24, con Francesco. **M5.5a**: in un clone temporaneo di Anagrafe, scollegato da GitHub, si legge il codice (le pagine sono 19, non 10) e si misura la convivenza. Aggiungere Tailwind e il tema v2 cambia gli stili di base di tutte le pagine — l'azzeramento di Tailwind, sfondo, testo e carattere sul `body` —, quindi «i due temi convivono» vale per i nomi dei token e va verificato per la resa. **M5.5b**: la guida e il passo 0. Il passo 0 passa da una PR nel repository di Anagrafe, secondo le sue regole (ramo, PR, squash; ogni push su `main` va in produzione), e lo approva Francesco. La guida dice anche come i documenti di Anagrafe (`CLAUDE.md`, `docs/INTERFACCE.md`, roadmap, checklist) passano dalle regole v1 al blocco v2: per un periodo convivono, pagina migrata per pagina migrata. Il testo qui sotto resta il riferimento per il contenuto della guida.*
