...
**M5.1 — `registry.json` completo (1 sessione)** — *rivisto il 2026-09-22: `registry validate` e `list` sono verdi da M4.6, e `check:riferimenti` (M4ter.10) più `check:registry-build` (M4ter.4bis) tengono in gate `registryDependencies` e artefatto. Resta ciò che nessun gate guarda: la verifica sistematica delle **`dependencies` npm** (`cn`, `lucide-react`, i pacchetti reui) sui **95** item (il conto al 2026-09-22; `public/r/` ne ha 96 file perché uno è l'indice) — possibilmente come estensione di `check:riferimenti`, se si trova un modo senza falsi positivi.*
