...
- **Ogni sessione chiude con**: `build-storybook`; le pagine riscritte aperte nel browser, in chiaro e in scuro; `test:a11y` con conto **invariato a 1524/0** (le descrizioni non entrano nel canvas: se il conto si muove, si è toccato altro); la riga in `CHECKLIST.md` e la voce in `WORKLOG.md`.
...
- Accettazione: `check:storybook -- --self-test` 7/0/0; le 7 pagine a **0 segnalazioni**; il gate stampa il conto sul resto del repo (atteso ≈740) senza fallire; `test:a11y` 1524/0.
...
- Accettazione: `check:storybook` a **0 segnalazioni** sui 26; `test:a11y` 1524/0.
...
- Accettazione: `check:storybook` a **0 segnalazioni** sui 52 file di `ui/`; `test:a11y` 1524/0.
...
- Accettazione: `check:storybook` a **0 segnalazioni** sui 21; `test:a11y` 1524/0; **la story di `data-table` nomina `altezza="ferma"`, la regola della misura relativa e la sequenza che riproduce il cricchetto**.
