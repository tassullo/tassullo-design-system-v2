...
**Negli alberi la ricerca parte dalle foglie, e il piè conta i due livelli (#80).** Con `getSottoRighe` il blocco accende `filterFromLeafRows`: una madre resta se lei o una discendente corrisponde. Una madre che corrisponde resta con le sole figlie che corrispondono, e i suoi subtotali sommano quelle: succedeva già col filtro dall'alto. Il testo del piè l'ha scelto Francesco fra tre varianti rese affiancate («4 voci», «4 voci, 5 misurazioni», «5 misurazioni»): la seconda, con la prop `nomeSottoRighe`; senza, il piè conta il solo primo livello come prima.
