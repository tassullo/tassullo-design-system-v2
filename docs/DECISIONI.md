...
14. **Tre rilievi di Francesco provando in Safari** (coda di M5.0d; le sette prove della lista di verifica le ha date buone, tranne la seconda).
    ...
    Resta un caso vicino, non segnalato: nel foglio la colonna «Designazione» mostra le misure rientrate e in corsivo (lo decide `mostra` della scena), e il campo in modifica no, quindi lì il testo si sposta di 16px a sinistra aprendo la modifica.
