...
5. **`sonner` e `spinner`, i due nomi che arrivano da fuori.** `sonner` risponde ad `Alt`+`T` (`hotkey = ['altKey', 'KeyT']`), annuncia in una regione `aria-live="polite"` e tiene un avviso 4 secondi; `spinner` porta di serie `aria-label="Loading"`, in inglese, e la pagina chiede di passarlo in italiano. *(Superato il 2026-09-25, proposta #55: lo `spinner` del registry si chiama «Caricamento» di serie, e le crocette di `dialog` e `sheet` «Chiudi».)*
...
## 64. v1 e v2 nella stessa app: convivono i nomi, non la resa. Strada D (M5.5a, 2026-09-24)

**Scelta da Francesco**, guardando le quattro strade rese e affiancate. Rettifica `PIANO.md` M5.5, che dava per dimostrato che «i due temi convivono senza collisioni di nomi» perché il v1 usa `--color-*` e il v2 `--primary`/`--background`: il ragionamento guardava i token del v2, non quelli **del tema di Tailwind**, che hanno gli stessi prefissi del v1.

**Il banco.** Un clone locale di Anagrafe, scollegato da GitHub. Tailwind e il tema v2 aggiunti seguendo `docs/INTEGRAZIONE.md` senza togliere il v1. Chromium di Playwright da una cartella temporanea, 1440×900, in chiaro. Per ogni elemento della pagina, gli stili calcolati (colori risolti dal motore di resa) confrontati con la pagina di oggi, elemento per elemento: **1.454 elementi** senza backend (18 rotte, 6 finestre modali aperte, il login) e, dopo la scelta e su richiesta di Francesco, **3.112 elementi** con risposte dell'API inventate e intercettate nel browser di prova (22 scene: liste con righe, badge, tab, schede di dettaglio, il dettaglio di un change set, l'amministrazione). Dall'altra parte, una pagina v2 di prova dentro Anagrafe, 20 sonde confrontate con la stessa pagina in un'app col solo v2.

**Tre fatti accertati.**

1. **Venti nomi li dichiarano tutti e due**: `--color-accent`, `--color-accent-ink`, `--color-border`, `--color-border-strong`, `--color-sidebar-border`, `--color-success`, `--color-success-border`, `--font-mono`, `--radius-sm/md/lg`, `--shadow-sm/md/lg/modal`, `--text-xs/sm/base/lg/xl`. Tailwind emette solo quelli che qualcosa usa, **diciassette** con le primitive installate per la prova (non `--color-accent-ink`, `--color-border-strong`, `--color-success`): il conto cresce coi componenti installati. Li emette nel livello `theme`, il v1 fuori da ogni livello, quindi **vince il v1**. Per i colori è la cosa giusta: in v2 `--color-accent` è il grigio dell'hover, e se vincesse il v2 i bottoni del v1 diventerebbero grigi. I colori, i raggi e le ombre del v2 non ne soffrono, perché stanno in `@theme inline` e le utility ne portano il valore già risolto. **La scala dei testi sì**: sta in un `@theme` semplice (§30, per la densità), quindi `text-sm` legge `--text-sm` e trova i 12px del v1 invece dei 13. Un diciottesimo nome entra per un'altra porta: le pagine di Anagrafe scrivono `var(--color-overlay, rgba(0,0,0,.4))`, un nome che il v1 non definisce e il v2 sì, e il ripiego smette di valere (lo sfondo delle modali v1 passa da nero 40% a #141414 45%, in tutte le strade).
2. **L'azzeramento di Tailwind cambia tutte le pagine v1.** Il v1 contava sui valori del browser: interlinea `normal`, titoli in grassetto, margini dei paragrafi. Seguendo `INTEGRAZIONE.md` si spostano **976 elementi su 1.454**: interlinea 1,5 dappertutto, i titoli perdono il grassetto, paragrafi e avvisi perdono i margini. Invece sfondo, colore e carattere del `<body>`, che il piano temeva, **non cambiano**: le regole del v1 sono fuori dai livelli e battono `@layer base`.
3. **Il v1 guasta anche il v2.** Nella stessa pagina v2 dentro Anagrafe, **0 sonde su 20** sono uguali al riferimento: niente Inter (il `<body>` del v1 impone il suo carattere), testi 1px più piccoli (punto 1), e il link perde l'arancio perché `a { color: inherit }` del v1, fuori dai livelli, batte le utility. **Senza azzeramento è peggio**: Arial su bottoni e campi, link sottolineati, margini. I componenti v2 contano sull'azzeramento.

**Le strade misurate.**

| | pagine v1 | pagina v2 dentro Anagrafe | costo |
|---|---|---|---|
| A · come `INTEGRAZIONE.md` | 976 elementi spostati su 1.454 | 0 sonde su 20 uguali | niente |
| B · Tailwind senza azzeramento | 0 spostati | 0 su 20, e peggio di A | tre righe |
| C · azzeramento fuori dalle zone v1 | **0 spostati** | 18 su 20: i menu nel portale restano col carattere e la scala del v1 | una ventina di righe nel CSS globale, tre attributi su ogni pagina migrata |
| **D · C + Inter e scala v2 su tutto il documento** | 971 spostati, **solo carattere e corpo**: Inter, testi +1px; colori, raggi, spazi, margini invariati | **20 su 20** uguali, menu compresi | come C, due attributi in `index.html`, **un** attributo per pagina migrata |

**La ricetta D**, nel CSS globale dell'app, senza togliere niente del v1:

- l'ordine dei livelli per primo, `@layer theme, base, v1, components, utilities;`, e Tailwind importato a pezzi: `theme.css` nel livello `theme`, `utilities.css` in `utilities`, **l'azzeramento dentro `@scope`**: `@scope (:root) to (<zone v1>) { @import "tailwindcss/preflight.css"; * { @apply border-border outline-ring/50; } }`. Tailwind accetta l'`@import` lì dentro e lo espande: l'azzeramento **non si copia**. Le zone v1 di Anagrafe sono la colonna, ogni figlio del contenuto che non porta `data-v2`, il login e la schermata d'attesa. La base v2 (`* { border-border }`) sta nello **stesso** `@scope`, perché a parità di livello una regola con `@scope` vince su una senza (la prossimità viene prima dell'ordine), e fuori il colore dei bordi v2 perdeva contro l'azzeramento;
- le regole di `<html>` dell'azzeramento **fuori** dallo `@scope`, perché lì dentro non lo raggiungono (un selettore dentro `@scope` è un discendente della radice, e `html` non è discendente di sé stesso): interlinea 1,5, `tab-size`, `text-size-adjust`, `font-sans`;
- le regole del v1 dell'app dentro `@layer v1`: sopra l'azzeramento, sotto le utility, così `a { color: inherit }` non batte più `text-accent-ink`; e `line-height: normal` sulle zone v1, che altrimenti ereditano l'1,5 di `<html>`;
- il tema del v1 (`@tassullo/theme/theme.css`) resta **fuori dai livelli**: deve continuare a vincere sui colori;
- in `index.html`, `<html data-density="normale">` e `<body class="font-sans">`. Il primo usa il blocco della densità che il tema ha già (§6): ridichiara la scala dei testi e vince sul `:root` del v1 perché viene dopo, a parità di peso. Tocca **solo** `--spacing` e `--text-*`, non i colori. Il secondo batte il carattere del `<body>` del v1 perché le utility stanno sopra `v1`;
- ogni pagina migrata porta **`data-v2`** sulla radice, e basta: provato che `data-density` e `font-sans` sulla pagina, necessari in C, in D sono superflui (20 sonde su 20 anche senza).

**La verifica con i dati finti**, nella strada D contro le pagine di oggi: su 3.112 elementi cambiano **solo** il carattere (2.853, Inter) e il corpo (1.910, +1px); l'interlinea cambia su `<html>`, `<body>` e i contenitori sopra le zone v1 (110), e 12 margini la seguono perché sono in `em` o `auto`. Nessun colore, grassetto, bordo, spaziatura o riempimento. Nella strada A, per confronto, si spostano 2.592 elementi e cambiano anche colori (73), grassetti (62), margini e spaziature dei campi.

**Perché D e non C**: C lascia le pagine v1 identiche, ma menu, tendine e dialoghi v2 si aprono in un portale in fondo al `<body>`, fuori dalla pagina migrata, e lì il carattere e la scala sono quelli del v1: 12px e niente Inter per tutto il tempo della transizione. D paga subito il prezzo sulle pagine v1, Inter e un pixel in più sui cinque gradini `xs`…`xl` (`--text-md` 13px e `--text-title` 26px, che il v2 non ha, restano), ed è la stessa direzione in cui andranno comunque.

**Da sapere**: il v1 di Anagrafe dichiara `Replicall` in testa alla pila, ma il carattere non è installato né distribuito, quindi oggi le pagine rendono nel carattere di sistema (San Francisco sul Mac, Segoe UI su Windows); D le porta a Inter. `@scope` vuole Chrome 118, Safari 17.4, Firefox 128. Il limite `to (…)` nomina classi dell'app: la ricetta è di **guida**, non un item del registry, e va scritta in `GUIDA-MIGRAZIONE.md` (M5.5b) con la regola per trovare le zone v1 di un'app qualunque.

**Seguito, lo stesso giorno: su Anagrafe v1 e v2 non convivono.** Deciso da Francesco dopo aver letto la misura. Il passaggio al v2 si fa **tutto su un ramo**: le pagine si riscrivono sul v2, il v1 (`@tassullo/theme`, le classi ricopiate nel CSS globale, i CSS di pagina) si toglie nello stesso lavoro, e in produzione arriva solo l'app finita, perché ogni push su `main` di Anagrafe va in produzione. Durante il passaggio sul repository di Anagrafe lavora **solo Francesco**, quindi un ramo lungo non si incrocia col lavoro di altri. La ricetta D resta scritta qui come misura e come ragione della scelta: far convivere i due temi costa una ventina di righe di CSS con `@scope` e livelli, e cambia comunque carattere e corpo di tutte le pagine v1. Cade per Anagrafe la migrazione incrementale che `PIANO.md` M5.5 dava come impianto della guida («ogni pagina migrata è un commit isolato», «ci si può fermare a metà»): la guida di M5.5b si scrive per il passaggio in un colpo solo. Per Studio e Officina la scelta si rifarà quando toccherà a loro.

## 65. Il passo 0 su un'app esistente: due file, e la CLI che fa più di quel che dice (M5.5b, 2026-09-24)

Provato su un clone di Anagrafe (frontend in `frontend/`, backend Python nella radice), con la CLI `shadcn@latest` 4.21.0. Tre fatti, tutti con una conseguenza su `docs/GUIDA-MIGRAZIONE.md`.

- **`npx shadcn@latest mcp init --client claude` non scrive solo `.mcp.json`**: aggiunge `shadcn` alle `devDependencies`. Nella radice di Anagrafe, che non ha un `package.json`, ne ha creato uno, con `package-lock.json` e 73 MB di `node_modules`. In un'app nuova non si nota, perché `init` ha già messo `shadcn` fra le dipendenze; in un'app esistente il passo 0 non sarebbe più «due file». **`.mcp.json` si scrive a mano.**
- **`shadcn mcp --cwd <cartella>` non ha effetto.** L'opzione è dichiarata, ma gli strumenti del server leggono `process.cwd()` (verificato nel sorgente della CLI): con `--cwd frontend`, `-c ./frontend` o il percorso assoluto, il registry `@tassullo` risulta «Unknown». Se il frontend sta in una sottocartella, il server va fatto **partire** da lì. Due lanciatori provati, entrambi funzionanti: `sh -c "cd frontend && exec npx shadcn@latest mcp"` e uno in Node (`process.chdir('frontend')` poi `spawn('npx', …, { shell: true })`). La guida porta quello in Node, perché non dipende da una shell POSIX e va quindi anche su Windows.
- **`shadcn init` davanti a un `components.json` esistente fa due domande, anche con `--yes`**: se sovrascriverlo, e se reinstallare i componenti. Con «no» alla prima esce con errore; con «sì» e nessuna risposta alla seconda esce con **codice 0 senza aver fatto niente**, lo stesso genere di difetto muto della conferma del tema (§60). Il `components.json` del passo 0 va quindi **cancellato prima di `init`**, e il registry si rimette come nel passo 4 di `docs/INTEGRAZIONE.md`.

**La prova.** Nel clone, `frontend/components.json` (lo stesso che userà la migrazione, registry compreso) e `.mcp.json` col lanciatore: `git status` mostra solo i due file nuovi. Interrogato il server su stdio, come un client, elenca `@shadcn` e `@tassullo`, trova 82 voci per «lista» e legge `tassullo-confirm-dialog`. Poi da una sessione vera di Claude Code (`claude -p` nella radice del clone, col solo `.mcp.json`): trova `tassullo-confirm-dialog`, `alert-dialog` e `tassullo-toast-con-annullo` cercando «conferma», e ne legge la descrizione. Nessun file di codice o di stile cambiato.

## 66. Il controllo per le app: si scrive qui, e le app lo installano (M5.7, 2026-09-24)

**Deciso da Francesco.** Col v1 le app avevano una regola scritta («solo variabili») e un linter che la faceva rispettare; col v2 avevano solo le regole, e nessuna verifica automatica. Il controllo **si scrive nel design system** e le app lo installano come ogni altro item: scritto da ciascuna app, sarebbe una copia che diverge, cioè esattamente ciò che la regola permanente vieta.

**Cos'è.** L'item `tassullo-controllo` porta un file solo, `scripts/tassullo-controllo.mjs`, senza dipendenze (Node 22 e la rete). La CI dell'app lo lancia a ogni push. Tre controlli:
1. **I file del design system sono intatti.** Il confronto con la versione di `components.json` lo fa **la CLI di shadcn** (`add … --dry-run`, poi `--diff` sui file che risultano da riscrivere), non un confronto nostro: la CLI trasforma i file installando (import, testa, `"use client"`), e rifarlo per conto nostro sarebbe una seconda implementazione che diverge alla prima versione nuova. È la stessa lezione di `check:registry-build`. Due conseguenze, entrambe misurate: la CLI esce con 0 anche quando i file differiscono, quindi si legge il testo; e un file da cui la CLI toglierebbe solo `"use client"` risulta «da riscrivere» senza esserlo, quindi quelle righe non contano.
2. **Nelle cartelle del design system stanno solo i suoi file.** In `ui/`, `blocks/` e `pages/` un file che non viene da `@tassullo` è un errore: una primitiva scritta in casa, o un componente installato da shadcn senza `@tassullo/`. Se quel componente Tassullo lo ha già, il caso lo prende il punto 1, perché il file differisce dal nostro; se non lo ha, lo prende questo. Un componente preso da shadcn identico al nostro (oggi `progress`, mai ri-stilato) non è un errore: è lo stesso file.
3. **Le regole di stile del blocco per il `CLAUDE.md`**, sul codice scritto dall'app: dieci regole, da valori arbitrari e esadecimali a `<select>` nativi, `Intl.NumberFormat` e `@tassullo/theme`, più `components.json` con `base-nova` e una versione fissata (non `main`). Una riga che deve fare eccezione porta `tassullo-controllo: <il motivo>`; il motivo è obbligatorio, e l'eccezione vale sulla sua riga o sotto un commento che sta da solo, non sulla riga dopo un'eccezione scritta in fondo.

**Come si è provato.** Autotest di 34 casi, messo fra i gate (`check:controllo`, l'undicesimo). Poi su un'app di prova installata da GitHub: pulita, passa; con violazioni messe apposta le prende tutte, 13 su 13: un file scritto a mano in `ui/`, un componente shadcn che Tassullo non ha, `badge` preso da shadcn al posto del nostro, un blocco ritoccato, nove regole di stile, e una riga con eccezione motivata che viene lasciata stare. Due difetti del primo giro, corretti prima di pubblicarlo: la riga d'avanzamento della CLI («- Resolving items.») letta come riga di diff, e un'eccezione che valeva anche per la riga dopo. Infine su una copia di Anagrafe ancora sul v1: installarlo aggiunge solo lo script (`package.json` invariato), e `--solo-stile` dà gli stessi conti letti a mano in M5.5a — 5 finestre del browser, 28 tendine native, 4 numeri formattati a mano — più 232 esadecimali nei ripieghi di `var(--…, #…)` dei CSS di pagina, che la regola del v1 vietava e nessun controllo vedeva. Per questo la guida di migrazione lo propone anche come **inventario**, prima del ramo.

**Il gate di questo repository** `check:riferimenti` ora legge anche gli import dei `.mjs` spediti, e non chiede di dichiarare i moduli di Node (`node:…`): prima uno script `.mjs` sarebbe passato senza controllo.

**La versione resta `v2.0.0`** (Francesco): l'etichetta si sposta sul commit che contiene il controllo, invece di aprire una `v2.1.0`. Si può fare senza danno solo perché nessuna app ha ancora installato niente con `v2.0.0`, pubblicata poche ore prima (Anagrafe ha solo `components.json`, dal passo 0). Da qui in avanti un'etichetta pubblicata non si sposta più: una modifica è una versione nuova.

## 67. Il lint delle app sui file installati (proposta #49, 2026-09-24)

**Il difetto.** Anagrafe, installata la `v2.0.0`, ha trovato tre errori del proprio `npm run lint` (ESLint del template Vite fino a `create-vite` 8: `typescript-eslint`, `eslint-plugin-react-hooks` 7, `eslint-plugin-react-refresh`) in file che non può modificare: `set-state-in-effect` su `use-mobile.ts`, `only-export-components` su `button.tsx` e `sidebar.tsx`. Il lint di questo repository è oxlint, che le regole di React 19 le dà come **avvisi**, e non è fra i gate: nessuno le guardava. Installati **tutti** i file del registry nello stesso modo, gli errori erano molti di più: 63 di `only-export-components` in 18 file, e altri 35 in 10 file.

**`use-mobile` diverge da shadcn, e di proposito.** Su `useSyncExternalStore` con `matchMedia`, come `use-soglia`: stessa firma (`useIsMobile(): boolean`), stessa soglia (768px), `false` nel render lato server come prima. Misurato in Chromium, vecchio contro nuovo: a 375px il vecchio fa due render, `false` e poi `true`, il nuovo uno solo, `true`; a 767 idem; a 768 e 1280 entrambi `false`; ridimensionando la finestra entrambi seguono. L'originale shadcn (`npx shadcn view @shadcn/use-mobile`) è ancora quello con l'effetto. È una divergenza di **forma**, quindi fuori dalla scala 4bis del `CLAUDE.md`, e l'ha chiesta Francesco con la proposta. Gli hook non hanno un originale in `registry/.upstream/`: alla prossima versione di shadcn si confronta a mano, ed è scritto nella testa del file.

**Le esportazioni: una riga nel lint dell'app, non un commento nei file.** Tre strade:
- *commento `eslint-disable` nei file*: una divergenza di forma su ogni componente che esporta varianti o hook (18 file fra `ui/` e `blocks/`), e un'app senza `eslint-plugin-react-refresh` fallirebbe con «Definition for rule … was not found» — lo stesso errore che `pdf-preview.tsx` dava già, con una direttiva per `import/no-unresolved`;
- *spostare `buttonVariants`, `useSidebar` e simili in file a parte*: cambia gli import delle app, cioè l'API, e diverge da shadcn;
- *una riga nella configurazione dell'app*: la regola spenta sulle sole cartelle `ui/` e `blocks/`. Scelta questa: l'effetto della regola è solo in sviluppo (il ricaricamento rapido ricarica la pagina intera quando cambia uno di quei file), e nell'app quei file non cambiano mai.

**Il codice di terzi: eccezioni per file e per regola.** Carosello (shadcn), calendario a eventi e stepper (reui) violano regole di React 19 (`set-state-in-effect`, `refs`, `purity`, e variabili inutilizzate nel calendario). Correggerli è una divergenza di forma da codice che si aggiorna copiandolo: le regole si spengono solo su quei file, e solo quelle. **I file nostri invece si sono corretti**: `data-grid` (sette `any` diventati `CaratteristicheTabella`), `calendario` (una variabile inutilizzata), `rich-text-editor` (un'assegnazione inutile), `pdf-preview` (la direttiva per una regola che nessun'app ha). E tre file di reui avevano l'import dell'icona **sopra** `"use client"`, dove l'aveva messo la CLI risolvendo il segnaposto: in un file la CLI poi non toglieva la direttiva, e ESLint la leggeva come un'espressione inutile. Rimessa in testa come nell'originale.

**La configurazione sta in un posto solo**: il passo 12 di `docs/INTEGRAZIONE.md`, per ESLint e per oxlint. Il gate `check:lint-app` (il dodicesimo) la legge da lì, installa con la CLI vera tutti i `.ts`/`.tsx` in un'app di prova, e fallisce su un errore o su un'eccezione che non serve più. Esito: ESLint 0 errori e 5 avvisi (`exhaustive-deps` sul codice di terzi, `incompatible-library` su `useReactTable`), oxlint 0 errori e 13 avvisi. Undici secondi.

**Perché non un item del registry con la configurazione.** Sarebbe coerente con «nessuna app tiene una copia», ma è un file nuovo per ogni app e un formato in più da mantenere; le righe sono poche e cambiano solo con una versione, e «Aggiornare» dice di ricopiarle. Se crescono, si riapre.

## 68. Nel rail i gruppi aprono un menu (proposta #50, 2026-09-24)

**Il difetto.** Con `collassa="icona"` e voci con `figli`, a colonna chiusa l'icona di un gruppo apriva e chiudeva un sottolivello che nel rail è nascosto: un clic che non faceva niente, e l'unica strada per le voci era riaprire la colonna. È la forma di `sidebar-07` di shadcn, che ha lo stesso difetto (`npx shadcn view @shadcn/sidebar-07`, `nav-main.tsx`). Anagrafe l'ha trovato sulla pagina modello `Pagine/Prodotti (Anagrafe)` e nel frattempo usa `collassa="fuori"`.

**La scelta (Francesco): un menu a destra dell'icona.** A colonna chiusa, sulla scrivania, l'icona di un gruppo è il grilletto di un `DropdownMenu` con `side="right"`: in cima il nome del gruppo, sotto le voci con i loro collegamenti (`render` o `href`), le disabilitate spente. A colonna aperta e sul telefono resta il `Collapsible`. Nessun componente nuovo e nessuna primitiva toccata: è la composizione che lo stesso `sidebar-07` usa per il menu utente, e `dropdown-menu` era già fra le dipendenze del guscio. Scartata la seconda forma della proposta, riaprire la colonna al clic: cambia la larghezza di tutta la pagina per scegliere una voce.

**La voce attiva: la spunta, non lo sfondo.** Tre varianti rese e confrontate in chiaro e in scuro: lo sfondo è lo stesso grigio del fuoco da tastiera, e con il fuoco su un'altra voce si vedevano due righe identiche; il testo arancio sarebbe stato un uso nuovo del colore del marchio. La spunta è il segno che il selettore del contesto usa già per la commessa attiva. Più `aria-current="page"` e `font-medium`.

**Lo sfondo del menu segue la modalità**, come gli altri due menu della colonna (selettore del contesto e utente). Un menu sempre scuro come la colonna è stato reso e scartato: in chiaro avrebbe dato un menu scuro e due bianchi nella stessa colonna, e i colori dentro i menu — le voci distruttive, le spunte, le voci che passa l'app — sono tarati sullo sfondo dei menu; su un fondo scuro il rosso distruttivo era già stato misurato a 3.52:1. Se un giorno si vorranno scuri, si decide per tutti e tre insieme.

**Misurato in Chromium** (Playwright, contro lo Storybook di sviluppo): il tooltip compare al passaggio e sparisce aprendo il menu; `Invio` e `Spazio` aprono, il fuoco va alla prima voce, le frecce scorrono, `Esc` chiude e riporta il fuoco sull'icona; clic e `Invio` su una voce arrivano all'ancora senza `preventDefault` e chiudono il menu; la voce disabilitata non si attiva. Riaprendo la colonna (`Ctrl`+`B`) tornano i gruppi apribili, e a 375px il pannello ha i gruppi apribili.

**Un secondo difetto, trovato misurando.** Nel rail l'etichetta di sezione della primitiva (`SidebarGroupLabel`) diventa trasparente e risale di tutta la sua altezza (`-mt-8`), senza lasciar passare il puntatore: copriva la metà bassa dell'ultima icona della sezione sopra, 16px su 32 in densità normale e 24 su 48 in touch. È nell'originale shadcn. Corretto nel blocco con `group-data-[collapsible=icon]:pointer-events-none` sull'etichetta, una classe, senza toccare la primitiva: rimisurato, 0px morti su tutte le icone in entrambe le densità.

**I gruppi ricordano se sono aperti o chiusi** (chiesto da Francesco). Il `Collapsible` si smonta quando la colonna si chiude a icone, perché al suo posto c'è il menu, e sul telefono ogni volta che il pannello si chiude: con lo stato dentro di sé, un gruppo chiuso a mano tornava aperto. Lo stato sta ora nel guscio, che resta montato per tutta la vita dell'app: un elenco dei gruppi chiusi, perché ogni gruppo parte aperto. Provato in Chromium: sulla scrivania un gruppo chiuso resta chiuso dopo `Ctrl`+`B` due volte; sul telefono resta chiuso dopo aver chiuso e riaperto il pannello. Non sopravvive a un ricaricamento della pagina, e non serve: nelle app la navigazione non ricarica.

## 69. L'ondata 1 delle proposte di Anagrafe (#58–#80, 2026-09-26)

Tredici proposte arrivate da Anagrafe il 2026-09-25 con causa e rimedio già scritti, lavorate in una PR sola, per `v2.0.4`. Qui le scelte che valgono oltre il singolo difetto; il resto è nel diario.

**La forma di un `ui/` non si cambia nemmeno per una prop di Base UI (#60).** Il velo di una conferma aperta dentro un dialogo manca perché Base UI non rende il `Backdrop` di un dialogo annidato senza `forceRender` (`@base-ui/react` 1.8.0: `enabled: forceRender || !nested`). Aggiungere la prop in `ui/alert-dialog.tsx` è una divergenza di forma, e `check:registry` l'ha rifiutata. La correzione sta nel blocco: `tassullo-confirm-dialog` compone da sé portale, velo con `forceRender` e pannello, con le classi di `AlertDialogContent` copiate. **Costo dichiarato**: quando shadcn ri-stila `AlertDialogContent`, il pannello del blocco va riallineato a mano, e nessun gate lo vede (stesso caso di `PopoverContentFerma` nei filtri). La forma d'uso è una sola: la conferma si rende **dentro** il contenuto del dialogo, così `Esc` chiude solo lei e il fuoco torna al grilletto; resa accanto, `Esc` chiude il dialogo sotto.

**Un valore arbitrario ereditato si sposta componendo, non riscrivendolo (#64).** Il filo della tab attiva (`after:bottom-[-5px]`, di shadcn) usciva di 1px dalla lista: con `overflow-x-auto` la lista scorreva in verticale. `bottom-[-4px]` è un valore arbitrario nostro, rifiutato dal gate; `-bottom-1` dipende da `--spacing` (6px in touch). Resta il valore di shadcn più `-translate-y-px`, 1px fisso in tutte e due le densità. Se shadcn corregge il suo `-5px`, il nostro `-translate-y-px` va tolto.

**Una container query non interroga l'elemento che la dichiara (#77).** `FieldGroup` è lui stesso `@container/field-group`: `@lg/field-group:` scritto sul `FieldGroup` cerca un contenitore sopra e non scatta mai, senza errore. La query si scrive sui figli. Controllate le altre container query con nome del registry: nessuna ha lo stesso errore.

**Le righe aperte restano aperte, se hanno un'identità (#71).** `tassullo-data-table` spegne `autoResetExpanded` quando c'è `idRiga` o la coppia nuova `righeEspanse`/`onRigheEspanseChange`. Senza `idRiga` la richiusura resta, voluta: l'id è la posizione, e dopo un caricamento la stessa posizione può essere un altro record. **È un cambio di comportamento predefinito** per chi passa già `idRiga`.

**Negli alberi la ricerca parte dalle foglie, e il piè conta i due livelli (#80).** Con `getSottoRighe` il blocco accende `filterFromLeafRows`: una madre resta se lei o una discendente corrisponde. Nella verifica Francesco ha scelto che una voce trovata per il suo nome resti con **tutte** le figlie (la ricerca negli alberi tiene una riga se corrisponde lei o un'antenata): prima restava con le sole figlie che contenevano il testo, cioè vuota. Il testo del piè l'ha scelto Francesco fra tre varianti rese affiancate («4 voci», «4 voci, 5 misurazioni», «5 misurazioni»): la seconda, con la prop `nomeSottoRighe`; senza, il piè conta il solo primo livello come prima.

**Le righe virtualizzate si misurano davvero (#76).** TanStack Virtual ricava l'indice da `data-index`: senza, non misurava niente e scriveva un avviso per riga. E misurare le righe montate non basta: lo spazio delle righe non montate resta alla stima, quindi la stima si prende dall'altezza vera della prima riga resa. Con righe di altezza molto diversa lo spazio resta approssimato.

**Altezza ferma (#58).** Con righe `snap-start` e intestazione ferma, senza `scroll-padding-top` Chromium aggancia da sé la prima riga sotto l'intestazione (`scrollTop` 40 all'apertura, senza che nessuno lo scriva). Lo scroll-padding viene dall'altezza misurata dell'intestazione; il tetto del riquadro si arrotonda per difetto a pixel interi.

**Il filtro non cambia il testo del dato (#72)**: senza `opzioni`, l'etichetta è il valore com'è. **La cella scelta si segna solo col fuoco (#66)**: il segno dipende dallo stato del fuoco della griglia, come già il bordo della cella attiva; l'ancora resta sulla prima cella perché è da lì che entra `Tab`. **Una cella scrivibile non abilitata si tratta come una calcolata (#75)**: esce dalla matrice delle frecce e si mostra come testo, non come campo disabilitato da attraversare.

**Dalla verifica con Francesco, a PR aperta.** Tre regole nuove, pagate con difetti veri.

- *Le prove non stanno nelle scene che si guardano.* Una `play` gira anche nel canvas: nella scena visibile muove il fuoco e riempie il pannello delle interazioni di passi che chi guarda non ha fatto. Da qui in poi una prova sta in una **scena di misura** gemella (`tags: ['!dev', '!autodocs']`, `...Scena`, `play`), come quelle del calendario: il gate la esegue, la barra e Docs no. Non vale per le `play` che si limitano ad aprire un popup perché il gate lo misuri: aprono lo stato che la scena vuole mostrare, e restano dove sono (deciso da Francesco).
- *Una misura del DOM non comanda un nuovo render a ogni render.* Il corpo virtualizzato rileggeva dopo ogni render «la prima riga» e, se diversa, chiamava `measure()`: in WebKit righe da 36,5px si arrotondano a 36 e 37 alternati, la prima riga cambiava a ogni giro e React si fermava col limite degli aggiornamenti. Chromium non lo mostra, e il gate gira solo lì. Una grandezza del DOM si segue con un `ResizeObserver` su un elemento fermo, non confrontandola a ogni render.
- *Una funzione di filtro di TanStack chiamata a mano vuole il valore risolto*: `fn(riga, id, fn.resolveFilterValue?.(valore) ?? valore)`, come dice la sua documentazione. Senza, la ricerca di serie confronta il minuscolo del dato col testo scritto dall'utente.
...
## 70. L'ondata 2 delle proposte di Anagrafe: le decisioni di forma (2026-09-26)

Le proposte in cui prima si decide la forma. Ogni scelta l'ha fatta Francesco su varianti rese affiancate in scene temporanee, fotografate in Chromium di Playwright (e misurate anche in WebKit dove c'è scorrimento o layout). Qui la scelta, le alternative scartate e la misura; il lavoro è nel diario.

**La fascia in alto resta ferma nel modo «scorre» (#69, variante A).** Nella dashboard a 1440×480, dopo 300px di scorrimento, la fascia stava a −300px (Chromium e WebKit, anche a 375). Ora la fascia è `sticky top-0` col fondo del guscio, ed è la forma di shadcn (blocco `sidebar-16`); scorre ancora la finestra, quindi PaginaGiù scorre appena aperta la pagina (392px in Chromium, 440 in WebKit) e il browser ricorda dove si era. Due difetti della forma, chiusi nel guscio: la colonna bloccata di una tabella passava sopra la fascia (21 punti di campionatura su 180) e si chiude con `isolate` sull'area del contenuto; l'elemento a fuoco tornando con Maiusc+Tab finiva sotto la fascia (6 su 15) e si chiude con uno `scroll-padding-top` sulla pagina. Resta uno scarto di 2px in WebKit sul bordo di un grafico. **È un cambio di comportamento predefinito**: in tutte le app la fascia diventa ferma. Scartate: (B) `contenuto="riempie"` più un'area che scorre composta dalla pagina, la forma di Anagrafe — due pezzi nuovi del guscio o una ricetta in ogni pagina, e senza un clic nel contenuto PaginaGiù non scorre (0px); (C) un valore nuovo `contenuto="scorre-sotto"`, stessi limiti di tastiera di B.

**Le tab di una scheda restano ferme sotto la fascia (#69, variante A).** Nella scheda di prova, dopo 400px, la barra delle tab stava a −336px. Ora la `TabsList` sta in una fascia `sticky` sotto quella della pagina, dentro il blocco della scheda, e scorre ancora la finestra; al cambio di tab la pagina torna in cima, altrimenti si resta a metà del pannello nuovo. Campo a fuoco coperto dalle tab: 0 su 15 in Chromium; in WebKit touch alcuni casi, sempre di 3px. Scartate: (B) tab fuori da un'area che scorre, la forma di Anagrafe (0 coperti, ma un secondo modo di scorrere solo per le schede); (C) tab `sticky` dentro un guscio «scorre-sotto» (2–4 coperti su 15, e chiedeva la C della fascia).

**Le tab che non stanno in riga diventano una `Select` (#63, variante A).** Sei tab in una finestra vera di 375px chiedevano 513px su 343 (touch 582 su 327): la pagina si allargava a 529px e scorreva di lato. La forma è una **ricetta** di due primitive che non si toccano: sotto una soglia del **contenitore** (`@container`, non la media query sulla finestra: segue la colonna aperta o chiusa e si misura nel canvas) la `TabsList` si spegne e compare una `Select` «Sezione: …» che guida le `Tabs` controllate. È la forma che Anagrafe aveva già scelto. Il limite, da scrivere nella scena: la soglia si sceglie a mano per ogni scheda. Shadcn non ha una forma per il troppo pieno delle tab (MCP: solo `tabs` e `scroll-area-horizontal-demo`). Scartate: (B) lista che scorre con una sfumatura ai bordi (a 375 si vedono 3 tab su 6, 2 in touch, e la tab a fuoco può restare nella sfumatura); (C) lista a capo (fino a 132px d'altezza in touch); (D) un menu «Altro» con le tab che non stanno, che sarebbe stato un componente nostro — non proposto, perché A, B e C bastavano.

**Una colonna può andare a capo: `meta.testo: 'tronca' | 'aCapo'` (#68, variante B).** Di serie `'tronca'`, com'era. A 1440 la colonna «Metodo di prova» aveva 9 celle tagliate su 10; a capo 0, righe da 37 a 37–73px. Un campo a valori e non un sì/no, così un terzo modo non chiederebbe un secondo booleano che contraddice il primo. Due regole, pagate con misure: **nella tabella virtualizzata l'opzione si ignora** (come già `pannelloRiga`), perché con righe di altezza diversa l'altezza totale salta mentre si scorre (108.313 → 144.784px) e in WebKit, dopo Fine, il fuoco finisce sul `body` e i tasti non rispondono più; e **una colonna a capo dichiara `larghezza`**, perché a 375 con `table-fixed` una colonna senza larghezza va a 0px e il testo diventa una torre (righe fino a 1.539px). Scartate: (A) `meta.aCapo`, il nome della issue, stessa resa; (C) anche `'dueRighe'` con `line-clamp-2`, che nascondeva di nuovo il testo (2 celle su 10) e non risolveva la virtualizzazione.

**Un Badge in una cella stretta si scrive con una ricetta (#68, variante A).** Il Badge è `inline-flex` centrato: in una cella stretta sbordava di 82px ed era tagliato di netto («EN 13501-1 clas»), e `max-w-full truncate` sul Badge lo tagliava ai due lati («3501-1 classifica»). La ricetta, documentata nella scena della tabella e in quella del Badge: `<Badge className="max-w-full"><span className="truncate">…</span></Badge>` → «EN 13501-1…». Il Badge che avvolge da sé i figli in uno span, la forma chiesta per prima dalla issue, `check:registry` la rifiuta (divergenza di forma, provata su una copia). Scartate: (B) il Badge ri-stilato con `max-w-full` e `[&>span]:truncate` — il gate lo accetta, ma cambia tutti i Badge e fuori dalle tabelle li peggiora (un Badge in un riquadro stretto, che oggi sborda leggibile, diventa «8-2 / CEN TR»); (C) un `CellaBadge` esportato dal blocco tabella, con il testo intero nel `title` — un pezzo in più da mantenere per un guadagno che c'è solo col mouse.

**Le coppie termine–valore si scrivono su un token di griglia (#78, variante D).** Nel tema entra `--grid-template-columns-termine: auto 1fr`, da cui Tailwind genera `grid-cols-termine`; la ricetta è `@container` › `dl grid grid-cols-1 gap-x-6 gap-y-1 @sm:grid-cols-termine`. Rende al pixel la forma che le scene usavano già (termine largo quanto serve, valore accanto) senza il valore arbitrario `grid-cols-[auto_1fr]` che `tassullo-controllo` rifiuta nelle app; sotto la soglia va in colonna (a 375 la forma di prima teneva il valore a 239px su 343 e una descrizione su 12 righe). È il **primo token di disposizione** del tema, generato da `scripts/hex-to-oklch.ts` come gli altri. Il suo difetto muto: con un tema vecchio `grid-cols-termine` non emette niente e non dà errore, come `text-md`; il gate lo deve sapere. Le scene che scrivevano la coppia ognuna a modo suo (tabella › Espansione, Drawer, Hover Card, Calendario, Foglio a gruppi) si allineano alla ricetta. Scartate: (A) `@sm:grid-cols-3` con il valore su due colonne, la forma di Anagrafe, senza token (il termine è sempre un terzo: 146px di vuoto coi termini corti, i lunghi a capo); (B) un componente nostro `DescriptionList` — l'MCP non trova in shadcn niente di simile, ma le ricette reggono, quindi la 4bis non lo giustifica; (C) `Item` di shadcn, sempre in colonna (+85% d'altezza) e con ruoli di elenco invece di `dl`.

**Le celle scrivibili della griglia restano come sono, per ora (#70, rinviata).** Nella griglia di prova con colonne miste le celle scrivibili erano identiche alle altre (stesso testo, nessun fondo, nessun bordo, stesso puntatore), e l'unico testo del blocco parla di copia e incolla. Rese quattro forme — fondo da campo (1,06:1 contro la card, e rovesciava la selezione), bordo da campo (`ring-input`, 1,36:1 in chiaro), sola lettura attenuata (5,37:1), e una riga d'aiuto che nomina le colonne scrivibili — e cercato come fanno gli altri: le griglie dell'ecosistema shadcn (Dice UI, ReUI, gridcn) **non segnano niente a riposo** e fanno scoprire la scrittura con carattere, Invio, F2 e doppio clic; AG Grid e MUI lasciano la scelta all'app; Glide sbiadisce le sole-lettura; Smartsheet mette un lucchetto nell'intestazione. **Francesco ha scelto di non fare nulla ora**, come le griglie shadcn: un aiuto su come si scrive potrà venire in seguito da un bottone «i», se servirà. La issue resta aperta con questa ragione.

**Il prezzo unitario della scena del foglio a gruppi si mostra in euro (variante C).** Nella scena `Righe di tipo diverso` i prezzi uscivano come arrivavano (0,62 / 38,5 / 1,8 / 4,2) e le virgole erano sfalsate di 8,44px; con `valuta()` si allineano (0px), come l'Importo e il prezzo della scena `Computo`. Vale per la **scena**: il formato di una colonna lo sceglie l'app con `mostra`, e il blocco non lo impone. Misurando è emerso un errore della scena, già in `v2.0.4`: `numero()` rileggeva il dato salvato con `leggiNumero`, che prende «2.375» per duemilatrecentosettantacinque (importo 44.887,50 € invece di 44,89 €; nel `Computo` «2.375,00 €»). Il dato salvato si legge con `Number()`, e `leggiNumero` resta al testo scritto. Scartate: com'è; `decimale()` a due cifre senza simbolo; da due a quattro decimali, che non nasconde cifre ma risfalsa le virgole sulle righe con più di due.

**Il calendario della cella data si chiude quando la cella scorre sotto la testata (variante B, provata a mano da Francesco).** Aperto il calendario e scorsa la griglia, la cella finiva sotto la testata ferma e il calendario restava sopra, coprendola fino a 40px, poi galleggiava staccato per circa 300px di scorrimento (Chromium e WebKit). Ora il blocco ascolta lo scorrimento del riquadro mentre il calendario è aperto e lo chiude appena la cella non sta più intera nell'area visibile; il fuoco torna al campo della data senza scatti, ↓ lo riapre. È la regola già scritta per il menu «⋯» di riga della tabella: si chiude, non si sposta. Tocca il blocco, non `ui/popover`. Scartate: com'è; nasconderlo restando aperto (il fuoco restava su un giorno invisibile e un tasto sceglieva una data alla cieca); la sola classe su `data-anchor-hidden` di Base UI, che non vede la testata ferma (copriva ancora 15–40px) e quando scatta fa perdere il fuoco.

**Un link con l'aspetto di un bottone è un `Link` (o un `<a>`) con `buttonVariants` (#73, variante A).** `Button` è il bottone di Base UI. Col `render` di un collegamento, com'era scritto nelle scene, Base UI stampa un errore in console e mette `type="button"` sull'`<a>` (40 su 40 nella tabella Prodotti). Con `nativeButton={false}`, la forma di Anagrafe in 28 punti, il collegamento prende `role="button"`: si apre anche con Spazio e `getByRole('link')` lo trova 0 volte su 40. La forma scelta è quella che shadcn documenta per il Button («As Link») e che Base UI indica: niente `Button`, la classe da `buttonVariants({ variant, size })` sul `Link` del router o su un `<a>`. Resta un link (40 su 40), 0 errori, aspetto identico (0 pixel di differenza). Nel registry si riscrivono l'azione con indirizzo di `page-header` (che nella forma stretta perdeva anche il collegamento) e due scene; `ui/pagination.tsx` resta com'è, perché è codice di shadcn. Scartate: (B) `nativeButton={false}`, che le due documentazioni vietano per i collegamenti; (C) un `LinkBottone` nostro, che la 4bis non permette quando il gradino 1 regge — il solo vantaggio era una migrazione meccanica dei 28 punti di Anagrafe.

**La guida dice «Un collegamento non è un `Button`» come regola generale (#73, variante B).** La regola sbagliata stava in tre posti (`INTEGRAZIONE.md`, le regole d'uso della tabella, la pagina del `Button`: «si usa `render` con un link»). Il paragrafo nuovo sta accanto a «Il nome dice la funzione, non l'aspetto», copre anche «Torna a…» e «Apri sul sito» (in Anagrafe 5 dei 28 punti non sono celle), e si ripete accorciato nelle due pagine. Nella pagina del `Button` c'è una scena «Come collegamento» con una prova che il link resti un link (`getByRole('link')`): né l'occhio (0 pixel di differenza) né axe (0 differenze fra le forme) vedono il difetto. Scartata: correggere la sola riga delle tabelle.

**`tassullo-controllo` rifiuta il `Button` usato come collegamento, leggendo il tag intero (#73, variante A).** È la prima regola del controllo che legge un tag d'apertura su più righe invece di una riga sola: scatta su un `<Button …>` che ha `render` di un `<a>` o di un `…Link`, con o senza `nativeButton`, e il messaggio indica `buttonVariants` sul `Link`. Il prototipo, in sola lettura su Anagrafe: 28 casi su 28 veri, 0 falsi allarmi; non scatta su `DropdownMenuTrigger render={<Button/>}`, `BreadcrumbLink render={<Link/>}` né su `Button render={<div/>} nativeButton={false}`, l'uso previsto da Base UI. **È un cambio di comportamento del controllo**: la CI di Anagrafe fallisce con 28 violazioni finché non migra, e va detto con la versione. Scartate: (B) una regola a riga su `nativeButton={false}` (non vede la forma senza `nativeButton`, e scatta sugli usi legittimi); (C) nessuna regola, che lascerebbe la guida senza chi la faccia rispettare.

**D14 riconfermata (2026-09-26).** Le app Tassullo sono strumenti interni e i lettori di schermo non sono un requisito. Riletta a proposito del #73, dove il solo effetto che dipende da D14 è il ruolo dei collegamenti resi come `Button` (annunciati come «pulsante», assenti dall'elenco dei link): la scelta del #73 non dipende da D14, perché la forma scelta è quella documentata. Restano quindi com'erano i link della paginazione di shadcn con `role="button"` e i bottoni-icona senza nome del `combobox`.

**Le pagine modello sono esempi da leggere e ricomporre, e restano nel registry (#62, variante A).** Le sei descrizioni dicevano «Pagina modello: si installa e si adatta», e la guida mostrava `PaginaLista` importata. Nei fatti Anagrafe, l'unica app sul v2, non ne ha importata nessuna (0 su 6: ogni pagina ricomposta con blocchi e primitive). Ora la regola è una: una pagina d'esempio mostra come si compongono i blocchi, si legge con `npx shadcn view` o dall'MCP e si ricompone nell'app; **non si installa e non si importa**. Cambiano solo parole: descrizioni e `docs` dei sei item, `INTEGRAZIONE.md` (l'esempio della lista rifatto con `AppShell`, `PageHeader` e `DataTable`), `GUIDA-MIGRAZIONE.md`, l'introduzione della style guide e i testi delle scene. Resta il canale da cui un'app, e l'agente che ci lavora, legge un esempio senza uscire dal progetto (`view` restituisce il codice intero). Il rischio che resta, scritto: `add` funziona ancora, e una pagina installata per sbaglio finisce in `components/pages`. Scartate: (B) solo scene della style guide (sei item tolti, dodici file spostati, quattro script e `tassullo-controllo` da toccare, e l'MCP non trova più esempi); (C) accesso ed errore da importare e le altre quattro scene (due regole invece di una, e Anagrafe non importa nemmeno quelle due).

**Nell'esempio di amministrazione un utente si disattiva con l'annullo (#65, variante B).** Tre correzioni. Due senza scelta: un solo conteggio, quello del piè che segue la ricerca (cercando «zzz» si leggevano «4 utenti» sopra e «0 utenti» sotto); la ricerca guarda anche le etichette dei ruoli («editor» 0 → 2, «Amministr» 0 → 1), con il segnaposto «Cerca nome, email o ruolo…». La terza scelta da Francesco: «Rimuovi» diventa «Disattiva»/«Riattiva» nel menu di riga; «Disattiva» non chiede niente, la riga dice subito «Disattivato» e l'avviso di `tassullo-toast-con-annullo` offre «Annulla» per 5 secondi, e la chiamata parte alla sua chiusura (annullo differito, D18). «Riattiva» è immediata, ed è la seconda strada per tornare indietro. La prop `onRimuovi(id)` diventa `onCambiaStato(id, attivo)`: incompatibile, ma nessuna app importa la pagina. L'app deve montare `<Toaster />`, o l'avviso non compare e l'azione non parte. Scartate: (A) conferma con dialogo, la forma scelta per Anagrafe (tre gesti, e un'eccezione a D18 da scrivere); (C) interruttore nella colonna Stato (un clic solo in una riga fitta, l'arancio ripetuto su ogni riga attiva, un pattern che nessun'altra tabella usa).

**`FileUpload` ha una forma compatta: `forma="bottone"` (#61, variante A).** Con il file già caricato la zona di rilascio restava alta 206px (291 in touch) sotto la riga dell'allegato. Ora `forma: "zona" | "bottone"`, di serie `"zona"`: la forma bottone rende un `Button` outline sm «Sostituisci» fra le azioni della riga, con lo stesso `useDropzone`, la stessa validazione e gli stessi messaggi; il trascinamento resta possibile sul bottone. Il blocco allegato più zona passa da 304 a 90px (1440 normale). Un tipo a unione rifiuta `etichetta` e `descrizione` con la forma bottone. Le app installate non cambiano. Due difetti muti trovati provando, e chiusi nella stessa forma: in WebKit il campo file nascosto di react-dropzone allargava il contenitore (pagina a 593px su 375), e si chiude con `absolute` sul campo e `relative` sul contenitore; in scuro lo stato «file trascinato sopra» non cambiava il bordo, perché `dark:border-input` del bottone outline vinceva, e si chiude con le stesse classi col prefisso `dark:`. Scartate: (B) un `FileUploadBottone` a parte, identico a vista ma con un nome in più; (C) una zona tratteggiata a una riga (153px, e con Tab Rimuovi viene prima di Sostituisci).

**Con la forma compatta, il file rifiutato si dice sotto la riga (#61).** Il blocco non mostra gli errori da sé: passa il motivo già tradotto a `onRifiutati` e decide l'app, com'era per la zona. La scena e la documentazione mostrano come uso consigliato un `Alert` `destructive` sotto la riga dell'allegato: resta accanto al file e non sparisce da solo (è ciò che fa Anagrafe). Scartati come consiglio: l'avviso a comparsa, che sparisce e a 375 copre la riga sotto (resta adatto a confermare una sostituzione riuscita), e una riga fallita di `FileUploadList`, che si legge come un secondo allegato.

**Inter arriva nelle app come CSS in `public/`, collegato da `index.html` (#79, variante C; aggiorna §15).** §15 prevedeva che, con due item separati, il carattere si scaricasse e restasse in cache per conto suo; nell'app non succedeva, perché Vite fonde l'`@import` nell'unico CSS dell'app. Misurato su una build vera di Anagrafe: CSS di 364 kB (174 kB brotli), di cui 204 kB di carattere, con un nome che cambia a ogni rilascio, quindi 174 kB riscaricati anche per una sola classe cambiata. Ora l'item `tema-font` mette lo stesso CSS col base64 in `public/`, con un nome che cambia solo quando cambia il carattere, e l'app lo collega con un `<link rel="stylesheet">` in `index.html` invece di importarlo: a un rilascio si riscaricano 21 kB; nessuno sfarfallio; resta testo (il canale del registry porta solo testo), resta la stessa origine (D3). Il difetto muto: se il link manca, il testo esce nel carattere di sistema senza errori, e `tassullo-controllo` lo verifica (il link c'è, punta al file installato, e il CSS globale non importa il carattere). La CLI, in `public/`, lascia la nota di testa del file: va tolta dal sorgente. Scartate: collegare da `index.html` il file in `src/` (Vite lo rifonde, 364 kB come prima); `.woff2` veri scritti da uno script dell'item, con preload (prima visita 94 kB invece di 175, ma uno script da lanciare a ogni aggiornamento, e se manca la build passa col carattere assente); il CSS importato con `?url` da `main.tsx` (testo senza Inter per 1,7 s alla prima visita).

**Il gate ha una passata WebKit, sulle sole scene di misura che scorrono e cambiano densità.** Il ciclo di Safari dell'ondata 1 esisteva solo in WebKit, e il gate girava solo in Chromium. Rimesso il codice di prima in una copia: le scene esistenti non lo trovano in nessuno dei due motori (nessuna scorre e cambia densità); con cinque scene di misura nuove che lo fanno, Chromium lo trova in 1 su 5, WebKit in 5 su 5, uguale in tre giri. Ora quelle scene portano un'etichetta, e un secondo progetto di vitest le esegue in WebKit: 14–15 s in più. WebKit su tutte le scene costava 41 s e non ha trovato niente in più. In CI WebKit si installa con le sue dipendenze di sistema (107 MB e 52 pacchetti; il tempo si misura alla prima PR). I gate diventano tredici. Scartate: solo Chromium; le scene di misura senza WebKit (1 su 5); WebKit su tutto.

**Due difetti di `v2.0.4` trovati provando, corretti senza scelta di forma (sì di Francesco).** *Griglia*: il testo delle celle scrivibili stava in cima al riquadro (`min-h-9` più padding) mentre quello della `TableCell` di sola lettura è centrato: 5px più in alto in touch, 0,72 in normale. `content-center` sulla vista e `items-center` sul riquadro di modifica e sul `SelectTrigger`: scarto 0, altezze invariate, testo fermo aprendo la modifica (Chromium e WebKit). *Caricamento file*: il campo file nascosto di react-dropzone era un elemento della colonna di `Empty` e aggiungeva un intervallo sopra (41/25px, touch 61/37); con il campo `absolute` e la zona `relative`, 25/25 e 37/37, zona −16px (−24 in touch).

**La selezione di più celle della griglia si segna come in ReUI: bordo attorno al rettangolo e fondo tenue (scelta di Francesco, variante D).** Il fondo di prima (`bg-accent/40` su ogni cella del rettangolo, cella attiva compresa) faceva 1,03:1 contro la card in chiaro e spariva sotto il puntatore (1,00:1). Ora il rettangolo ha un lato di 1px sul perimetro (`border-primary` col fuoco dentro, `border-muted-foreground` col fuoco fuori, come l'anello della cella attiva di §69) e un fondo `bg-primary/4`, la ricetta di ReUI presa com'è; la cella attiva non ha fondo, e una cella sola non ha né fondo né bordo, come nei fogli di calcolo. Il fondo in chiaro resta quasi impercettibile (1,02:1): il segno lo porta il bordo (1,91:1 in chiaro, 8,78:1 in scuro). **Cambio di comportamento**: la cella attiva perde il fondo. Scartate: grigio pieno (si confondeva col grigio del puntatore e nella riga attiva si capovolgeva), fondo arancio senza bordo (in chiaro si leggeva solo per la tinta), bordo con fondo arancio al 15%. Emerso: la griglia seleziona solo da tastiera, né trascinando né con Maiusc+clic.

**Una colonna senza larghezza non scende sotto un minimo (variante B).** Con `table-fixed`, quando le colonne dichiarate non stanno nel riquadro la colonna senza larghezza andava a 0px e il suo titolo si sovrapponeva al vicino: a 375 (tabella 592px su 341, e 888 su 325 in touch), e in touch anche a 768; lo mostrava già la scena pubblicata `Stretta`. Ora il blocco dà alla tabella una `min-width` pari alla somma delle larghezze `w-<n>` dichiarate più 40 unità di `--spacing` per ogni colonna senza larghezza (160px, 240 in touch), e sotto quella il riquadro scorre di lato con lo scorrimento che aveva già. Sulla scrivania la tabella resta identica al pixel. È uno stile in linea calcolato da `var(--spacing)`, la stessa unità delle utility `w-*`, come l'altezza ferma che il blocco già calcola: scritto qui perché la regola 3 lo sappia. Se una larghezza non è `w-<n>`, il minimo non si calcola e resta il comportamento di prima. **Cambio di comportamento predefinito**: una tabella la cui colonna elastica scenderebbe sotto 160px ora scorre invece di schiacciarla. Con la colonna a capo di #68 a 375 le righe restano fra 37 e 110px, non più una torre. I documenti dicono anche che, per le liste pensate per il telefono, la risposta è la faccia a schede che esiste già. Min-width sulle celle o su `<col>` non regge (WebKit li ignora). Scartate: ogni colonna dichiara una larghezza (sulla scrivania il browser ridistribuiva lo spazio: la colonna larga da 654 a 265px); colonne nascoste sotto una soglia (in touch tornava a 0px); solo la faccia a schede (la tabella restava col difetto).

**Dalla verifica con Francesco, a PR aperta.**

- *La ricetta termine–valore ha una variante per i riquadri piccoli.* In un riquadro stretto e fisso con valori corti — un'anteprima, un riepilogo di cifre — le coppie restano affiancate: `grid grid-cols-termine`, senza `@container` e senza la soglia. Sotto la soglia, nella Hover Card larga 236px, la ricetta intera le mandava in colonna. Le misure di laboratorio di una scena non sono coppie da mostrare e non seguono la ricetta.
- *Le classi di `buttonVariants` passano per `cn`.* Da sole lasciano il `border-transparent` della base accanto al bordo della variante `outline`, e vince il primo: il link esce senza bordo, senza errore. `Button` non ha il problema perché fa da sé il `cn`. `tassullo-controllo` lo segnala nelle app (`classi-del-bottone-senza-cn`). Un confronto a pixel su una scena sola non l'aveva visto: il difetto stava nelle varianti che quella scena non usava.
- *Tab strette nella scheda d'esempio:* con tre tab che a 375 ci stanno in riga, Francesco ha scelto di tenere la `Select` sotto `@sm` anche lì.
- *Gli esempi di codice delle pagine d'esempio* mostrano la composizione coi blocchi, non il componente d'esempio importato.
