...
- `npm run check:contrast` — verifica il rapporto di contrasto delle **26 coppie** di `scripts/hex-to-oklch.ts`, **uscendo con codice 1 sotto 4.5:1**. Erede di `stylelint-config.cjs` del v1: senza un controllo automatico la regola si perde in poche settimane. **La palette Tassullo vive dentro quello script**, in una sola costante: è la fonte unica, e il CSS del tema è il suo output — `-- --css` emette i blocchi `oklch`, `-- --self-test` dimostra che il gate sa fallire. La conversione degli hex non si fa a mano. Mappa ragionata e rilievi: `PIANO.md` §2bis.
...
- `npm run theme:build` — rigenera `registry/tassullo/theme/tassullo-theme.css` dalla palette di `scripts/hex-to-oklch.ts`. **Quel CSS è un file generato e non si modifica a mano**: `check:contrast` fallisce se diverge dall'output dello script. Un token nuovo si aggiunge alla palette, non al CSS.
