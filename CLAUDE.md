...
4bis. **Di un componente shadcn si cambiano SOLO le stringhe di classi.** Struttura, props, nomi delle varianti e delle taglie, export: quelli di shadcn, identici. Non è purismo: i componenti non sono una dipendenza, sono codice **copiato**, e una versione nuova va riportata a mano. Finché le divergenze sono tutte dentro le stringhe di classi, riportarla è un lavoro fattibile; una variante o un prop in più non si distingue più da ciò che hanno cambiato loro. `registry/.upstream/` conserva il sorgente originale di ogni componente e `npm run check:registry` fa rispettare la regola — **non si modifica `.upstream/` a mano**. Se serve davvero un'aggiunta strutturale: si fa un componente **nuovo** che avvolge quello shadcn, non lo si altera.
