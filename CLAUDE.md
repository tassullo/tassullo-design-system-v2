...
- **I numeri da confrontare in colonna vogliono `tabular-nums`, non un altro font.** La style guide del v1 diceva «monospace per codici sistema *e dati tabellari*»: la seconda metà **è caduta**. Replicall porta la feature OpenType `tnum`, e le sue cifre tabellari misurano 580/1000 di em su tutte e otto le facce — quindi il totale in grassetto si incolonna col corpo in tondo. Misurato in `Tema/Cifre`: scarto dei decimali **2.38px senza, 0px con**. Il `font-mono` resta ai **codici di sistema**, dove si *vuole* che stonino. Avvertenza: `tnum` non tocca la punteggiatura — virgola e punto restano proporzionali e cambiano larghezza col peso, quindi la larghezza dell'intera stringa non coincide fra tondo e grassetto anche quando le cifre sono perfettamente allineate. Il righello giusto è il bordo dei decimali, non la virgola.
