...
const ISTRUZIONI = `  /* ════════════════════════════════════════════════════════════════════
     ...
     la palette di partenza: i blocchi \`@theme inline\`, \`:root\` e \`.dark\`,
     e l'\`@import\` del carattere Geist. Stanno DOPO questo \`@import\` e
     quindi VINCONO: vanno tolti tutti e quattro, o i colori e il carattere
     Tassullo non si vedono. Questo file ridichiara tutto ciò che serve.
...
${colorMap}
  ...
  /* ── Tipografia ──────────────────────────────────────────────────────
     **Inter** è il carattere delle interfacce, scelto su un confronto
     ...
     Font, quindi gratuito e ridistribuibile. Arriva col tema, in un CSS a
     parte che l'app tiene in public/ e collega da index.html, senza
  ...
  /* ── Coppie termine–valore (\`<dl>\`).
     Genera \`grid-cols-termine\`: la colonna del termine larga quanto serve,
     il valore accanto. Si scrive \`grid grid-cols-1 @sm:grid-cols-termine\`
     su un \`<dl>\` dentro un \`@container\`, e sotto la soglia va in colonna.
     Con un tema che non lo dichiara la classe non emette niente, senza
     errore: il termine e il valore finiscono uno sotto l'altro. */
  --grid-template-columns-termine: auto 1fr;
}
