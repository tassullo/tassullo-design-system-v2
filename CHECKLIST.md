...
**Due rilievi aperti da M2.2, entrambi in carico a chi è indicato.**

1. **Il contorno dei controlli sta sotto 3:1 (WCAG 1.4.11), su tutto il set e in entrambe le modalità.** Misurato: `border-input` su pagina **1.27 in chiaro e 1.36 in scuro**; su card 1.36 e 1.25. Non viene dal ri-stile — è il valore di `--input`/`--border`, ereditato dal v1 — e `check:contrast` non lo vede perché verifica **solo coppie di testo**. **In carico a M2.9**, ed è una decisione sulla palette (la fonte unica in `hex-to-oklch.ts`), non sui componenti.
2. **Il `select` aperto non è verificabile nel pannello del browser**: `requestAnimationFrame` non ci scatta, e Base UI ci schedula dentro lo spostamento del fuoco nel popup — quindi frecce e `Invio` non rispondono, mentre col mouse la scelta funziona. **In carico a M2.3**, dove il comportamento da tastiera dei popup È il criterio di accettazione, e va provato in un browser vero.
