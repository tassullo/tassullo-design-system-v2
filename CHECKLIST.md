...
| D12 `next-themes` va tolto da `sonner`? | TODO | nessuno | Aperta da **M2.3**. `sonner.tsx` importa `useTheme` da `next-themes`, la libreria di temi di Next.js: noi la modalità la mettiamo come classe sulla radice. Senza il suo provider `useTheme()` ricade su `"system"`, cioè sul tema del **sistema operativo**. Conseguenza misurata: la descrizione del toast restava `#3f3f3f` su fondo scuro, **1.62:1** — chiusa con una stringa di classi (`**:data-[description]:text-muted-foreground!`, ora **7.17:1**), ma il pacchetto resta dichiarato fra le dipendenze dell'item, cioè **ogni app consumer lo installa per non usarlo**. Toglierlo è una modifica **strutturale** (una chiamata e una prop in meno), fuori dal gradino 2 della regola 4bis: non si fa di iniziativa. Misure in `WORKLOG.md` del 2026-09-09 |
