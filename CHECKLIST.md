...
| M3.3 `data-table` (**2 sessioni**) | **IN_PROGRESS** — sessione 1 di 2 chiusa 2026-09-10 | M2.4, M2.6 | ✔ **3 blocchi, 57 item, 916 scansioni / 0 violazioni.** TanStack Table v9 (9.2.4): colonne loro, contorno nostro; `componenti-propri.json` resta vuoto. Tastiera provata in Chromium — 1 Tab alla ricerca (500→**59** righe), 4 Tab all'intestazione, Invio ×3 e l'ordine di partenza torna. ✖ **resta la sessione 2**: il degrado a 375px non è progettato (la tabella scorre, 746→918px in 341 di riquadro) e il grilletto «Colonne» fa **41.56px** in touch. Coda: intestazione su `bg-accent` (l'hover fantasma è `muted`, su `muted` sparirebbe) e `table-fixed` con larghezze nel `meta` — le colonne non saltano più su ordinamento/pagina/filtro, misurato. → dettaglio in `WORKLOG.md`, voci **M3.3 (1 di 2)** e code **(1)** e **(2)**, e `docs/DECISIONI.md` §33. Nuovo item `toni` (58): i quattro toni semantici come `className`, che è la forma che shadcn documenta — nessuna primitiva toccata |
...
| D17 resize delle colonne da parte dell'utente | **APERTA** 2026-09-11 — la porta è tenuta aperta, non si costruisce | quando una tabella lo chiederà davvero | Aperta da Francesco: «non abbiamo idea di che tabelle svilupperemo in futuro, potrebbe diventare necessario **limitatamente a desktop**». TanStack v9 ce l'ha (`columnSizingFeature` + `columnResizingFeature`); shadcn no — la maniglia è **gradino 4**. **Verificato che M3.3 non la preclude**: `table-fixed` è un prerequisito, non un ostacolo. Il solo nodo è che il resize produce **pixel**, che la regola 3 vieta e che non seguono la densità. → `docs/DECISIONI.md` §34 |
