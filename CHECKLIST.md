...
| D13 il marchio Tassullo come item del registry | TODO | **entro la fine della FASE 2** (M2.9) | Aperta in M2.5. La sidebar vuole il marchio in testata — e nel rail la sola **T** — ma il logo è un file del brand, non una classe: distribuirlo è una **voce del registry** (`tema-logo`, accanto a `tema` e `tema-font`), non una story. Il wordmark del sito è un `<path>` unico con `fill="#141414"` — che è **esattamente** il token `--sidebar`, conferma indipendente della palette — e la T da sola non se ne estrae senza tagliare a mano i dati vettoriali. **Francesco fa mandare a Roberto la T bianca su fondo trasparente** (2026-09-09). Fino ad allora le story portano un **segnaposto disegnato in `currentColor`**, che rispetta la regola 3 (niente hex nei `fill`) e va sostituito, non lasciato. Da chiudere: formato dell'asset, come viaggia nel registry, e se il marchio completo serve accanto alla sola T |
