...
2. **`aria-hidden-focus` su ogni `dropdown-menu` aperto (6 nodi) e su `select` aperto (4 nodi), in entrambe le modalità — 12 violazioni, e non chiudibili a livello di ri-stile** (M2.3). I nodi sono i **guardiani del fuoco di Base UI** (`data-base-ui-focus-guard`), cioè il meccanismo che fa girare il `Tab` dentro al menu: generati dalla libreria, assenti da ogni stringa di classi. Il `dialog` monta gli stessi guardiani e lì axe li marca *incomplete*, perché portano anche `data-base-ui-inert`. **M2.9 deve decidere se esentare la regola**, o la CI nasce rossa il primo giorno.
3. **La voce di menu è il bersaglio più piccolo del set in densità touch: 36px** (voce di `command` 42), contro i 48 di bottone, campo e select. Passa WCAG 2.5.8 ma sta sotto i 44 del criterio di M2.9. Rimedio misurato: `py-1` → **`py-2`** = 48px esatti in touch, 33 in normale contro 25 — ma alza tutti i menu **anche alla densità da scrivania**, quindi è una scelta di sistema, come le costanti della sidebar.
