...
| D18 conferma o annullo, per quali azioni | **APERTA** 2026-09-11 — **si decide ora, si costruisce in M3.5** | **M3.5** (verdetto) | Aperta da Francesco su `confirm-dialog`: «un alert o sonner con la possibilità per l'elimina di annullarlo?». Non sono complementari, sono **alternative**: messi sulla stessa azione fanno due interruzioni e svuotano la conferma. Quale dei due lo decide il dato, e Anagrafe la regola ce l'ha già — «tutto ciò che è dichiarato verso l'esterno non si cancella mai, **si supera**» — quindi la gran parte delle «eliminazioni» sono passaggi di stato reversibili, cioè il caso da annullo. **Domanda aperta per Francesco**: per le cancellazioni vere, l'API farà un soft delete ripristinabile o l'annullo va differito lato client? Il posto è M3.5 (lo standard di §1.1 di Anagrafe copre anche il **successo**, e quel paragrafo impone «UN solo pattern prima di usarlo»). → `docs/DECISIONI.md` §35 |
